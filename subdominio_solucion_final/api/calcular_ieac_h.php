...
try {
    ...
    // Consulta SQL completa para calcular IEAC(h) con CPI6m × SPI
    $sql = "SELECT
    ...
    try {
        ...
        // Debug detallado
        error_log("🔍 IEAC(h) Debug - Proyecto: $proyecto_id, Fecha: $fecha_filtro");
        error_log("🔍 IEAC(h) Resultado SQL completo: " . json_encode($result));
        ...
        if ($result) {
            ...
            // Calcular IEAC(h) = Real + Por Ganar / (CPI6m × SPI)
            $ieac_h = $ac + ($por_ganar / ($cpi_6m * $spi));
            
            error_log("🔍 IEAC(h) Valores calculados:");
            error_log("  - BAC: $bac");
            error_log("  - AC: $ac");
            error_log("  - EV: $ev");
            ...
            error_log("  - IEAC(h): $ieac_h");
            
            $ieac_h_redondeado = round($ieac_h);
            
        } else {
            $ieac_h_redondeado = 0;
            error_log("❌ IEAC(h) No se obtuvo resultado de la consulta");
        }
        
    } catch (PDOException $e) {
        error_log("❌ Error ejecutando consulta IEAC(h): " . $e->getMessage());
        $ieac_h_redondeado = 0;
        $result = null;
    }
    ...
    echo json_encode([
        ...
        'debug' => [
            ...
            'ieac_h_calculado' => isset($result['ac']) && isset($result['cpi_6m']) && isset($result['pv_para_spi']) && isset($result['bac']) && isset($result['ev']) ? 
                (floatval($result['ac']) + ((floatval($result['bac']) - floatval($result['ev'])) / (floatval($result['cpi_6m']) * (floatval($result['ev']) / floatval($result['pv_para_spi']))))) : 'NULL',
            'valor_redondeado' => $ieac_h_redondeado
        ]
    ]);
