...
if ($proyecto_id) {
    $deleteStmt = $pdo->prepare("DELETE FROM real_acumulado WHERE proyecto_id = ?");
    $deleteStmt->execute([$proyecto_id]);
    error_log("🧹 Tabla real_acumulado limpiada para proyecto_id: " . $proyecto_id);
}
...
foreach ($rows as $row) {
    ...
    $stmt = $pdo->prepare("INSERT INTO real_acumulado (proyecto_id, centro_costo, periodo, tipo, cat_vp, detalle_factorial, monto)
        VALUES (?, ?, ?, ?, ?, ?, ?)");
