...
if ($proyecto_id) {
    $deleteStmt = $pdo->prepare("DELETE FROM api_acumulada WHERE proyecto_id = ?");
    $deleteStmt->execute([$proyecto_id]);
    error_log("🧹 Tabla api_acumulada limpiada para proyecto_id: " . $proyecto_id);
}
...
foreach ($rows as $row) {
    ...
    $stmt = $pdo->prepare("INSERT INTO api_acumulada (proyecto_id, centro_costo, periodo, tipo, cat_vp, detalle_factorial, monto)
        VALUES (?, ?, ?, ?, ?, ?, ?)");
