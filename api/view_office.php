<?php
// Endpoint específico para visualizar archivos Office (Word, Excel, PowerPoint) en iframe
// Este archivo permite que los visores externos accedan a los archivos
...
try {
    ...
    if (!in_array($extension, $extensiones_office)) {
        http_response_code(400);
        header('Content-Type: application/json');
        echo json_encode(['error' => 'Este endpoint solo sirve archivos Office (Word, Excel, PowerPoint)'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    // Determinar el tipo MIME correcto si no está en la BD
    if (empty($tipo_mime)) {
        switch ($extension) {
            case 'doc':
            ...
            case 'xlsx':
                ...
                break;
            case 'ppt':
                $tipo_mime = 'application/vnd.ms-powerpoint';
                break;
            case 'pptx':
    ...
    // Headers específicos para archivos Office (SIN X-Frame-Options para permitir iframe)
    header('Access-Control-Allow-Origin: *');
    header('Access-Control-Allow-Methods: GET, POST, OPTIONS');
    ...
    // Manejar Range requests (necesario para algunos navegadores y visores)
