...
if ($proyecto_id) {
    $deleteStmt = $pdo->prepare("DELETE FROM v0_parcial WHERE proyecto_id = ?");
    $deleteStmt->execute([$proyecto_id]);
    error_log("🧹 Tabla v0_parcial limpiada para proyecto_id: " . $proyecto_id);
}
...
foreach ($rows as $row) {
    ...
    $stmt = $pdo->prepare("INSERT INTO v0_parcial (proyecto_id, centro_costo, periodo, tipo, cat_vp, detalle_factorial, monto)
        VALUES (?, ?, ?, ?, ?, ?, ?)");
