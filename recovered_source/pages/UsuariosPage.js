...
import '../styles/UsuariosPageZoom.css';
  ...
  const handleConfirmarEliminarUsuario = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setUsuarios(usuarios.filter(u => u.id !== modalEliminarUsuario.id));
        setModalEliminarUsuario(null);
        setPasswordEliminar('');
        setErrorPasswordEliminar('');
        setFeedback({ type: 'success', msg: 'Usuario eliminado correctamente.' });
      } else {
