...
const ProyectoPage = () => {
  ...
  // Leer parámetro carpeta de la URL para navegación desde notificaciones
  useEffect(() => {
    const carpetaParam = searchParams.get('carpeta');
    if (carpetaParam) {
      // Cambiar a la vista de la carpeta específica
      setAnalisisSeleccionado(`carpeta_${carpetaParam}`);
    }
  }, [searchParams]);

  // Memoizar usuario y centros para evitar re-renders innecesarios
  const user = useMemo(() => {
    return JSON.parse(localStorage.getItem('user')) || { nombre: 'Usuario', rol: 'usuario' };
  }, []);
