...
const Vectores = ({ proyectoId }) => {
  ...
  const calcularIEAC = async (AC, EV, BAC, CPI, SPI, fechaSeguimiento, datosHistorico = []) => {
    ...
    // Calcular CPI por períodos si tenemos datos históricos (para otras metodologías)
    ...
    try {
      // Obtener IEAC(a) desde el backend
      const responseA = await fetch(`${API_BASE}/calcular_ieac_a.php?proyecto_id=${proyectoId}&fecha_filtro=${fechaSeguimiento}`);
      const dataA = await responseA.json();
      const ieacA = dataA.success ? dataA.ieac_a : 0;
      console.log('🔍 IEAC(a) desde backend:', ieacA, 'Data completa:', dataA);

      // Obtener IEAC(b) desde el backend
      const responseB = await fetch(`${API_BASE}/calcular_ieac_b.php?proyecto_id=${proyectoId}&fecha_filtro=${fechaSeguimiento}`);
      const dataB = await responseB.json();
      const ieacB = dataB.success ? dataB.ieac_b : 0;
      console.log('🔍 IEAC(b) desde backend:', ieacB, 'Data completa:', dataB);

      // Obtener IEAC(c) desde el backend
      const responseC = await fetch(`${API_BASE}/calcular_ieac_c.php?proyecto_id=${proyectoId}&fecha_filtro=${fechaSeguimiento}`);
      const dataC = await responseC.json();
      const ieacC = dataC.success ? dataC.ieac_c : 0;
      console.log('🔍 IEAC(c) desde backend:', ieacC, 'Data completa:', dataC);

      // Obtener IEAC(d) desde el backend
      const responseD = await fetch(`${API_BASE}/calcular_ieac_d.php?proyecto_id=${proyectoId}&fecha_filtro=${fechaSeguimiento}`);
      const dataD = await responseD.json();
      const ieacD = dataD.success ? dataD.ieac_d : 0;
      console.log('🔍 IEAC(d) desde backend:', ieacD, 'Data completa:', dataD);

      // Obtener IEAC(e) desde el backend
      const responseE = await fetch(`${API_BASE}/calcular_ieac_e.php?proyecto_id=${proyectoId}&fecha_filtro=${fechaSeguimiento}`);
      const dataE = await responseE.json();
      const ieacE = dataE.success ? dataE.ieac_e : 0;
      console.log('🔍 IEAC(e) desde backend:', ieacE, 'Data completa:', dataE);

      // Obtener IEAC(f) desde el backend
      const responseF = await fetch(`${API_BASE}/calcular_ieac_f.php?proyecto_id=${proyectoId}&fecha_filtro=${fechaSeguimiento}`);
      const dataF = await responseF.json();
      const ieacF = dataF.success ? dataF.ieac_f : 0;
      console.log('🔍 IEAC(f) desde backend:', ieacF, 'Data completa:', dataF);

      // Obtener IEAC(g) desde el backend
      const responseG = await fetch(`${API_BASE}/calcular_ieac_g.php?proyecto_id=${proyectoId}&fecha_filtro=${fechaSeguimiento}`);
      const dataG = await responseG.json();
      const ieacG = dataG.success ? dataG.ieac_g : 0;
      console.log('🔍 IEAC(g) desde backend:', ieacG, 'Data completa:', dataG);

      // Obtener IEAC(h) desde el backend
      const responseH = await fetch(`${API_BASE}/calcular_ieac_h.php?proyecto_id=${proyectoId}&fecha_filtro=${fechaSeguimiento}`);
      const dataH = await responseH.json();
      const ieacH = dataH.success ? dataH.ieac_h : 0;
      console.log('🔍 IEAC(h) desde backend:', ieacH, 'Data completa:', dataH);

      // Obtener IEAC(i) desde el backend
      const responseI = await fetch(`${API_BASE}/calcular_ieac_i.php?proyecto_id=${proyectoId}&fecha_filtro=${fechaSeguimiento}`);
      const dataI = await responseI.json();
      const ieacI = dataI.success ? dataI.ieac_i : 0;
      console.log('🔍 IEAC(i) desde backend:', ieacI, 'Data completa:', dataI);
      ...
      // IEAC(a): Costo real + trabajo por ganar - DESDE BACKEND
      // const ieacA = AC + porGanar;

      // IEAC(b): Presupuesto / CPI actual - DESDE BACKEND
      // const ieacB = BAC / CPI;

      // IEAC(c): Real + (Por Ganar / CPI actual) - DESDE BACKEND
      // const ieacC = AC + (porGanar / CPI);

      // IEAC(d): Real + (Por Ganar / CPI 3 meses) - DESDE BACKEND
      // const ieacD = AC + (porGanar / CPI3m);

      // IEAC(e): Real + (Por Ganar / CPI 6 meses) - DESDE BACKEND
      // const ieacE = AC + (porGanar / CPI6m);

      // IEAC(f): Real + (Por Ganar / (CPI × SPI)) - DESDE BACKEND
      // const ieacF = AC + (porGanar / (CPI * SPI));

      // IEAC(g): Real + (Por Ganar / (CPI3m × SPI)) - DESDE BACKEND
      // const ieacG = AC + (porGanar / (CPI3m * SPI));

      // IEAC(h): Real + (Por Ganar / (CPI6m × SPI)) - DESDE BACKEND
      // const ieacH = AC + (porGanar / (CPI6m * SPI));

      // IEAC(i): Real + (Por Ganar / (a×CPI + b×SPI)) - Ponderado (70% CPI, 30% SPI) - DESDE BACKEND
      ...
      console.log('🔍 Valores finales IEAC:', {
        ieacA: ieacA,
        ieacB: ieacB,
        ieacC: ieacC,
        ieacD: ieacD,
        ...
        ieacH: ieacH,
        ieacI: ieacI,
        ieacA_millones: (ieacA / 1000000).toFixed(2) + 'M',
        ieacB_millones: (ieacB / 1000000).toFixed(2) + 'M',
        ieacC_millones: (ieacC / 1000000).toFixed(2) + 'M',
        ieacD_millones: (ieacD / 1000000).toFixed(2) + 'M',
        ieacE_millones: (ieacE / 1000000).toFixed(2) + 'M',
        ieacF_millones: (ieacF / 1000000).toFixed(2) + 'M',
        ieacG_millones: (ieacG / 1000000).toFixed(2) + 'M',
        ieacH_millones: (ieacH / 1000000).toFixed(2) + 'M',
        ieacI_millones: (ieacI / 1000000).toFixed(2) + 'M'
      });

      return {
        // Metodologías individuales
        metodologiaA: ieacA, // Desde backend
        metodologiaB: ieacB, // Desde backend
        metodologiaC: ieacC, // Desde backend
        metodologiaD: ieacD, // Desde backend
        metodologiaE: ieacE, // Desde backend
        metodologiaF: ieacF, // Desde backend
        metodologiaG: ieacG, // Desde backend
        metodologiaH: ieacH, // Desde backend
        metodologiaI: ieacI, // Desde backend
      ...
      console.error('Error calculando IEAC desde backend:', error);
  ...
  const calcularIndicadoresEVM = async (data, fechaSeguimiento) => {
    ...
    const IEAC = await calcularIEAC(AC, EV, BAC, CPI, SPI, fechaSeguimiento, tablaRealParcial);
