...
// CRÍTICO: Limpiar cualquier output antes de enviar headers
if (ob_get_level()) {
    ob_end_clean();
}
ob_start();

// Headers CORS solo para OPTIONS
...
try {
    ...
    // CRÍTICO: Limpiar TODOS los buffers antes de enviar headers
    while (ob_get_level()) {
        ob_end_clean();
    }
    
    // Enviar headers en el orden correcto
    ...
    // Deshabilitar compresión de salida
    if (function_exists('apache_setenv')) {
        @apache_setenv('no-gzip', '1');
    }
    @ini_set('zlib.output_compression', 'Off');
    ...
    // Leer y enviar en chunks para evitar problemas de memoria
    while (!feof($handle)) {
    ...
    // Limpiar buffers antes de enviar error
    while (ob_get_level()) {
        ob_end_clean();
    }
