...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Log para debugging
    error_log("[carpetas.php] POST crear carpeta - nombre: $nombre, proyecto_id: $proyecto_id, carpeta_padre_id: " . ($carpeta_padre_id ?? 'null') . ", creado_por: $creado_por");
    ...
    if (empty($nombre) || !$proyecto_id || !$creado_por) {
        error_log("[carpetas.php] POST ERROR: Datos incompletos - nombre: '$nombre', proyecto_id: $proyecto_id, creado_por: $creado_por");
        ob_clean();
        http_response_code(400);
        sendCorsHeaders();
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['success' => false, 'error' => 'Datos incompletos', 'debug' => ['nombre' => $nombre, 'proyecto_id' => $proyecto_id, 'creado_por' => $creado_por]], JSON_UNESCAPED_UNICODE);
        exit;
    }
    
    try {
        // Verificar que el proyecto existe
        $stmt_check = $pdo->prepare("SELECT proyecto_id, nombre FROM proyectos WHERE proyecto_id = ?");
        $stmt_check->execute([$proyecto_id]);
        $proyecto_data = $stmt_check->fetch();
        if (!$proyecto_data) {
            error_log("[carpetas.php] POST ERROR: Proyecto no encontrado - proyecto_id: $proyecto_id");
            ob_clean();
            http_response_code(404);
            sendCorsHeaders();
            header('Content-Type: application/json; charset=utf-8');
            echo json_encode(['success' => false, 'error' => 'Proyecto no encontrado', 'debug' => ['proyecto_id' => $proyecto_id]], JSON_UNESCAPED_UNICODE);
            exit;
        }
        error_log("[carpetas.php] POST - Proyecto encontrado: " . ($proyecto_data['nombre'] ?? 'sin nombre'));
        
        // Verificar que la carpeta padre existe (si se especifica)
        if ($carpeta_padre_id) {
            try {
                $stmt_check = $pdo->prepare("SELECT id, nombre, proyecto_id, activo FROM carpetas WHERE id = ? AND activo = 1");
                $stmt_check->execute([$carpeta_padre_id]);
                $carpeta_padre_data = $stmt_check->fetch();
                if (!$carpeta_padre_data) {
                    error_log("[carpetas.php] POST ERROR: Carpeta padre no encontrada o inactiva - carpeta_padre_id: $carpeta_padre_id");
                    ob_clean();
                    http_response_code(404);
                    sendCorsHeaders();
                    header('Content-Type: application/json; charset=utf-8');
                    echo json_encode(['success' => false, 'error' => 'Carpeta padre no encontrada o inactiva', 'debug' => ['carpeta_padre_id' => $carpeta_padre_id]], JSON_UNESCAPED_UNICODE);
                    exit;
                }
                // Verificar que la carpeta padre pertenece al mismo proyecto
                if ($carpeta_padre_data['proyecto_id'] != $proyecto_id) {
                    error_log("[carpetas.php] POST ERROR: Carpeta padre pertenece a otro proyecto - carpeta_padre_id: $carpeta_padre_id, proyecto_carpeta: " . $carpeta_padre_data['proyecto_id'] . ", proyecto_solicitado: $proyecto_id");
                    ob_clean();
                    http_response_code(400);
                    sendCorsHeaders();
                    header('Content-Type: application/json; charset=utf-8');
                    echo json_encode(['success' => false, 'error' => 'La carpeta padre pertenece a otro proyecto', 'debug' => ['carpeta_padre_id' => $carpeta_padre_id, 'proyecto_carpeta' => $carpeta_padre_data['proyecto_id'], 'proyecto_solicitado' => $proyecto_id]], JSON_UNESCAPED_UNICODE);
                    exit;
                }
                error_log("[carpetas.php] POST - Carpeta padre encontrada: " . ($carpeta_padre_data['nombre'] ?? 'sin nombre') . " (ID: $carpeta_padre_id)");
            } catch (PDOException $e) {
                error_log("[carpetas.php] POST ERROR al verificar carpeta padre: " . $e->getMessage());
                throw $e;
            }
        }
        ...
        error_log("[carpetas.php] POST - Intentando INSERT carpeta: nombre='$nombre', proyecto_id=$proyecto_id, carpeta_padre_id=" . ($carpeta_padre_id ?? 'null'));
        ...
        error_log("[carpetas.php] POST - Carpeta creada exitosamente - ID: $carpeta_id");
        ...
        $errorCode = $e->getCode();
        error_log("[carpetas.php] POST ERROR PDO: $errorMessage (Código: $errorCode)");
        error_log("[carpetas.php] POST ERROR Stack trace: " . $e->getTraceAsString());
        ob_clean();
        http_response_code(500);
        sendCorsHeaders();
        header('Content-Type: application/json; charset=utf-8');
        ...
        error_log("[carpetas.php] POST ERROR Exception: " . $e->getMessage());
        error_log("[carpetas.php] POST ERROR Stack trace: " . $e->getTraceAsString());
        ob_clean();
        http_response_code(500);
        sendCorsHeaders();
