        ...
        if ($stmt_check->rowCount() === 0) {
            ...
            try {
                ...
                if ($id_info && strpos($id_info['EXTRA'], 'auto_increment') === false) {
                    ...
                    $pdo->exec("ALTER TABLE carpeta_linea_base_mitigadores MODIFY COLUMN id INT AUTO_INCREMENT PRIMARY KEY");
                    error_log("Corregido: campo 'id' ahora tiene AUTO_INCREMENT en carpeta_linea_base_mitigadores");
                }
    ...
    try {
        ...
        foreach ($linea_base as $index => $item) {
            ...
            if ($item_id && in_array($item_id, $existing_ids)) {
                ...
                error_log("[carpeta_linea_base_mitigadores] Intentando INSERT para carpeta_id: $carpeta_id, item index: $index");
                try {
                    // Intentar obtener el siguiente ID manualmente como fallback
                    $next_id = null;
                    try {
                        $stmt_max_id = $pdo->query("SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM carpeta_linea_base_mitigadores");
                    ...
                    try {
                        ...
                        error_log("[carpeta_linea_base_mitigadores] Modo SQL estricto desactivado temporalmente");
                    } catch (PDOException $e_mode) {
                        error_log("[carpeta_linea_base_mitigadores] Advertencia: No se pudo desactivar modo SQL estricto: " . $e_mode->getMessage());
                    ...
                    try {
                        ...
                        error_log("[carpeta_linea_base_mitigadores] Advertencia: No se pudo restaurar modo SQL estricto: " . $e_restore->getMessage());
                    }
                    ...
                    if ($nuevo_id) {
                        ...
                        error_log("[carpeta_linea_base_mitigadores] INSERT exitoso, ID agregado: $nuevo_id");
                    } else {
                        error_log("[carpeta_linea_base_mitigadores] ERROR: No se pudo obtener ID después del INSERT");
                    }
                    
                } catch (PDOException $e_insert) {
                    // Si el INSERT falla, intentar con ID explícito
                    error_log("[carpeta_linea_base_mitigadores] Primer INSERT falló: " . $e_insert->getMessage() . " - Intentando con ID explícito");
                    try {
                        // Restaurar modo SQL estricto antes del segundo intento
                        try {
                            $pdo->exec("SET SESSION sql_mode = ''");
                        } catch (PDOException $e_mode2) {
                            error_log("[carpeta_linea_base_mitigadores] Advertencia al desactivar modo SQL en segundo intento: " . $e_mode2->getMessage());
                        }
                        ...
                        $stmt_insert_id = $pdo->prepare("INSERT INTO carpeta_linea_base_mitigadores (
                            id, carpeta_id, control_mitigador_id, codigo, dimension, pregunta, evidencia, verificador_responsable,
                            fecha_verificacion, implementado_estandar, accion_ejecutar, responsable_cierre,
                            fecha_cierre, criticidad, porcentaje_avance, nombre_dueno_control, ultimo_usuario_edito,
                            estado_validacion, comentario_validacion, usuario_validacion, fecha_validacion, ponderacion,
                            comentario_trabajador, archivos_respaldo, conversacion_seguimiento,
                        ...
                        try {
                            ...
                            error_log("[carpeta_linea_base_mitigadores] Advertencia al restaurar modo SQL en segundo intento: " . $e_restore2->getMessage());
                        }
                        ...
                        if ($nuevo_id) {
                            $updated_ids[] = intval($nuevo_id);
                            error_log("[carpeta_linea_base_mitigadores] INSERT con ID explícito exitoso, ID agregado: $nuevo_id");
                        }
                        ...
                        try {
                            ...
                            error_log("[carpeta_linea_base_mitigadores] Error al restaurar modo SQL después de fallo: " . $e_restore->getMessage());
                        }
                        error_log("[carpeta_linea_base_mitigadores] ERROR CRÍTICO: Ambos intentos de INSERT fallaron. Error: " . $e_insert_id->getMessage());
        ...
        if (empty($updated_ids) && !empty($linea_base)) {
            error_log("[carpeta_linea_base_mitigadores] ADVERTENCIA CRÍTICA: updated_ids está vacío después del commit. Buscando IDs reales...");
            foreach ($linea_base as $index => $item) {
                $control_id = $item['control_mitigador_id'] ?? null;
                ...
                if ($control_id && $dimension && $pregunta && $evidencia) {
                    ...
                    if ($found && $found['id']) {
                        ...
                        if (!in_array($id_encontrado, $updated_ids)) {
                            ...
                            error_log("[carpeta_linea_base_mitigadores] ID encontrado por propiedades y agregado: $id_encontrado");
                        }
                    } else {
                        error_log("[carpeta_linea_base_mitigadores] ERROR: No se encontró registro con propiedades: carpeta_id=$carpeta_id, control_id=$control_id, dimension=$dimension");
                    }
                }
            }
        }
        ...
        if (empty($updated_ids) && !empty($linea_base)) {
            error_log("[carpeta_linea_base_mitigadores] ADVERTENCIA: updated_ids sigue vacío. Buscando último ID insertado para carpeta_id: $carpeta_id");
            $stmt_last = $pdo->prepare("SELECT id FROM carpeta_linea_base_mitigadores WHERE carpeta_id = ? AND activo = 1 ORDER BY id DESC LIMIT 1");
            $stmt_last->execute([$carpeta_id]);
            $last = $stmt_last->fetch(PDO::FETCH_ASSOC);
            if ($last && $last['id']) {
                $updated_ids[] = intval($last['id']);
                error_log("[carpeta_linea_base_mitigadores] Último ID encontrado y agregado: {$last['id']}");
            }
        }
        
        error_log("[carpeta_linea_base_mitigadores] POST exitoso para carpeta_id: $carpeta_id, items procesados: " . count($linea_base) . ", updated_ids: " . count($updated_ids) . " (" . implode(', ', $updated_ids) . ")");
        echo json_encode(['success' => true, 'message' => 'Línea base mitigadores guardada exitosamente', 'updated_ids' => $updated_ids], JSON_UNESCAPED_UNICODE);
    } catch (PDOException $e) {
        if (isset($pdo) && $pdo->inTransaction()) {
            try {
                $pdo->rollBack();
            } catch (PDOException $e_rollback) {
                error_log("[carpeta_linea_base_mitigadores] Error al hacer rollback: " . $e_rollback->getMessage());
            }
        }
        $error_message = 'Error al guardar línea base mitigadores: ' . $e->getMessage();
        $error_code = $e->getCode();
        error_log("[carpeta_linea_base_mitigadores] ERROR en POST: $error_message (Código: $error_code)");
        error_log("[carpeta_linea_base_mitigadores] Stack trace: " . $e->getTraceAsString());
        http_response_code(500);
        echo json_encode([
            'success' => false, 
            'error' => $error_message,
        ...
        $error_message = 'Error inesperado al guardar línea base mitigadores: ' . $e->getMessage();
        error_log("[carpeta_linea_base_mitigadores] EXCEPCIÓN GENERAL en POST: $error_message");
        error_log("[carpeta_linea_base_mitigadores] Stack trace: " . $e->getTraceAsString());
        http_response_code(500);
        echo json_encode([
            'success' => false, 
