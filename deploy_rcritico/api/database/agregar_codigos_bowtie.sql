-- Script para agregar campos de código a las tablas del análisis Bowtie
-- Permite numeración automática: CA1, CA2... para causas, CO1, CO2... para consecuencias, etc.
-- NOTA: Si las columnas ya existen, estos comandos fallarán silenciosamente (lo cual está bien)

-- Agregar campo código a causas
ALTER TABLE bowtie_causas 
ADD COLUMN codigo VARCHAR(20) NULL DEFAULT NULL COMMENT 'Código de la causa (ej: CA1, CA2...)';

-- Agregar campo código a consecuencias
ALTER TABLE bowtie_consecuencias 
ADD COLUMN codigo VARCHAR(20) NULL DEFAULT NULL COMMENT 'Código de la consecuencia (ej: CO1, CO2...)';

-- Agregar campo código a controles preventivos
ALTER TABLE bowtie_controles_preventivos 
ADD COLUMN codigo VARCHAR(20) NULL DEFAULT NULL COMMENT 'Código del control preventivo (ej: CCP1, CCP2...)';

-- Agregar campo código a controles mitigadores
ALTER TABLE bowtie_controles_mitigadores 
ADD COLUMN codigo VARCHAR(20) NULL DEFAULT NULL COMMENT 'Código del control mitigador (ej: CCM1, CCM2...)';

-- Agregar índices para búsquedas rápidas por código
-- Nota: Si el índice ya existe, el comando fallará silenciosamente
...
-- Tabla de relación: Controles Preventivos - Causas (muchos a muchos)
...
-- Tabla de relación: Controles Mitigadores - Consecuencias (muchos a muchos)
