...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    $id = intval($data['id'] ?? 0);
    $nombre_original = trim($data['nombre_original'] ?? '');
    
    if (!$id || !$nombre_original) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'id y nombre_original son requeridos']); exit;
    }
    
    try {
        $stmt = $pdo->prepare("UPDATE archivos SET nombre_original = ? WHERE id = ? AND activo = 1");
        $stmt->execute([$nombre_original, $id]);
        
        if ($stmt->rowCount() > 0) {
            echo json_encode(['success' => true, 'nombre_original' => $nombre_original]); exit;
        } else {
            http_response_code(404);
            echo json_encode(['success' => false, 'error' => 'Archivo no encontrado']); exit;
        }
    } catch (PDOException $e) {
        http_response_code(500);
